...
internal class VisitDetailReport
{
    ...
    internal static void ProcessVisitDetailsReports(string importDirectory, string temporaryDirectory)
    {
        ...
        ReportProcessor.ProcessExcelFiles(importDirectory, "*Visit_Details*.xlsx", (worksheet, sheetName) =>
        {
            if (sheetName.Equals("Meeting Details", StringComparison.OrdinalIgnoreCase))
            {
                ProcessWorksheet.ProcessSimpleKeyedSheet(worksheet, meetingDetailsById, meetingDetailsHeaders, "Meeting ID");
            }
            else if (sheetName.Equals("Participant Details", StringComparison.OrdinalIgnoreCase))
            {
                ProcessWorksheet.ProcessSimpleKeyedSheet(worksheet, participantDetailsByName, participantDetailsHeaders, "Participant Name");
            }
        });

        ReportProcessor.WriteSimpleJson(temporaryDirectory, "Visit_Details-Meeting_Details.json", meetingDetailsById);
        ReportProcessor.WriteSimpleJson(temporaryDirectory, "Visit_Details-Participant_Details.json", participantDetailsByName);
    }
}
